...
const { MEMORY_PATH, GROQ_API_KEY, OPENAI_API_KEY } = require("../config/config");
...
const analyzeVision = async (images) => {
  try {
    ...
    if (openai) {
      // Use OpenAI GPT-4o for vision with base64
      const messages = [
        {
          role: "user",
          content: [
      ...
      // Fallback to Groq with uploaded images
      ...
      console.error("No AI service available for vision");
      return "";
    }
  } catch (error) {

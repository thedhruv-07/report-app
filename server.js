      ...
      new TableRow({
        children: [
          ...
          new TableCell({
            ...
            children: [
              ...(logoRun
                ? [new Paragraph({ children: [logoRun], alignment: "center" })]
          ...
          new TableCell({
            ...
            children: [new Paragraph({ children: [new TextRun({ text: "Client Name (abbr.):", bold: true, size: 18 })] })],
          }),
          // Value 1
          new TableCell({
            width: { size: 25, type: "pct" },
            ...
            margins: { top: 20, bottom: 20, left: 40, right: 40 },
            children: [new Paragraph({ children: [new TextRun({ text: sanitizeDocxText(header.client || "-"), bold: true, size: 18 })] })],
          }),
          // Conclusion Header
          new TableCell({
            ...
            children: [new Paragraph({ children: [new TextRun({ text: "Conclusion", bold: true, size: 18 })], alignment: "center" })],
          }),
        ],
      }),
      // ROW 2
      new TableRow({
        children: [
          ...
          new TableCell({
            ...
            margins: { top: 20, bottom: 20, left: 40, right: 40 },
            children: [new Paragraph({ children: [new TextRun({ text: "Inspection Number:", bold: true, size: 18 })] })],
          }),
          // Value 2
          new TableCell({
            borders: {
            ...
            margins: { top: 20, bottom: 20, left: 40, right: 40 },
            children: [new Paragraph({ children: [new TextRun({ text: sanitizeDocxText(header.inspectionNumber || "-"), bold: true, size: 18 })] })],
          }),
          // Conclusion Status (Restart)
          new TableCell({
            verticalMerge: VerticalMergeType.RESTART,
            borders: {
              top: { style: BorderStyle.SINGLE, size: 6, color: "1F1F1F" },
            ...
            margins: { top: 180, bottom: 0, left: 40, right: 40 },
            children: [
              new Paragraph({
                children: [
                  new TextRun({
                    text: sanitizeDocxText(header.conclusion || "Pending"),
                    bold: true,
                    size: 40,
                    color:
                      header.conclusion === "FAILED" || header.conclusion === "Pending" || !header.conclusion
                        ? "FF0000"
      ...
      new TableRow({
        children: [
          ...
          new TableCell({
            ...
            margins: { top: 20, bottom: 20, left: 40, right: 40 },
            children: [new Paragraph({ children: [new TextRun({ text: "Report Date:", bold: true, size: 18 })] })],
          }),
          // Value 3
          new TableCell({
            borders: {
            ...
            margins: { top: 20, bottom: 20, left: 40, right: 40 },
            children: [new Paragraph({ children: [new TextRun({ text: sanitizeDocxText(header.reportDate || "-"), bold: true, size: 18 })] })],
          }),
          // Conclusion Status (Continue)
          new TableCell({
            verticalMerge: VerticalMergeType.CONTINUE,
            children: [],
          }),
...
7. 本报告为完整内容，不得部分复制本报告。`;
  ...
  if (hasGroupedPhotos) {
    const reportPhotoParagraphs = getGroupedPhotoGridParagraphs(data.reportPhotoGroups);
...
function getGroupedPhotoGridParagraphs(photoGroups) {
  ...
  // Filter groups that have at least one valid photo
  const validGroups = photoGroups
    .map((group) => ({
      description: group.description || "",
      photos: (Array.isArray(group.photos) ? group.photos : []).filter(
        (p) => p && typeof p.preview === "string" && p.preview.startsWith("data:image")
      ),
    }))
  ...
  validGroups.forEach((group, groupIdx) => {
    // Description Row for this group
          ...
          new TableCell({
            ...
            children: [
              new Paragraph({
                children: [
                  new TextRun({
                    text: sanitizeDocxText(descriptionText),
                    size: 18,
                    bold: true,
                    color: "1F1F1F",
                  }),
    ...
    // Photos for this group in rows of 2
    for (let i = 0; i < group.photos.length; i += 2) {
      const left = group.photos[i];
      const right = group.photos[i + 1] || null;
      rows.push(
